...
if (process.argv.length > 3) {
    try {
        ...
        fs.writeFileSync('teamstep.sql', ` INSERT INTO teamstep (Id, teamid, stepid, season) VALUES ` + 
            result.TeamStep.reduce((prev, ts) => prev + `\n` + 
            `(${ts.Id},${ts.TeamId},${ts.StepId},${ts.Season}),`, '') + `\n;`);

        fs.writeFileSync('person.sql', `INSERT INTO person (Id, Name, Gender, Birthdate, IdCardNr, VoterNr, Phone, Email) VALUES ` +
             result.Person.reduce((prev, p) => prev + `\n ` +
            `(${p.Id},${p.Name ? "'" + p.Name + "'": "null"},${p.Gender ? "'" + p.Gender + "'": "null"},${p.Birthdate ? "'" + p.Birthdate + "'": "null"},${p.IdCardNr ? "'" + p.IdCardNr + "'": "null"},${p.VoterNr ? "'" + p.VoterNr + "'": "null"},${p.Phone ? "'" + p.Phone + "'": "null"},${p.Email ? "'" + p.Email + "'": "null"}),`, '')+ `\n;`);

        fs.writeFileSync('player.sql', `INSERT INTO player (Id, Season, Resident, PersonId, TeamId, StepId, RoleId, CaretakerId) VALUES ` + 
            result.Player.reduce((prev, p) => prev + `\n ` +
            `(${p.Id},${p.Season},${p.Resident},${p.PersonId},${p.TeamId},${p.StepId},${p.RoleId},${p.CareTakerId}),`, '')+ `\n;`);
