//const storage = require('../db/storage');
const mysql = require('../db/mysql');

function getPersonById(id) {
    mysql.executeStatement('SELECT * FROM person WHERE Id = ?', [id])
        .then(results => { return { recordset: results, rowsAffected: [results.length] }});
}

// function getPersonById(id) {
//     return storage.getSingle('Person', id);
// }

function getPersonByIdCardNr(idCardNr) {
    return mysql.executeStatement('SELECT * FROM person WHERE IdCardNr = ?', [idCardNr])
        .then(res => { return { recordset: res, rowsAffected: [res.length] } });
}

// function getPersonByIdCardNr(idCardNr) {
//     //console.log('Search DocId: ' + idCardNr);
//     const query = function (db) {
//         const person = db.get('Person')
//             .cloneDeep()
//             .find({ IdCardNr: idCardNr })
//             .value();
//         //console.log('Get storage person:',person);
//         if (person) {
//             return { recordset: [ person ], rowsAffected: [ 1 ] };
//         }
//         else {
//             return { recordset: [], rowsAffected: [ 0 ] };
//         }
//     };
//     return new Promise((resolve, reject) => {
//         try {
//             const result = storage.statementQuery(query);
//             resolve(result);
//         }
//         catch(err) {
//             reject(err);
//         }
//     });
// }

function addPerson(name, gender, birthdate, docId, voterNr, phone, email, isLocalBorn) {
    return mysql.executeStatement('INSERT INTO person (Name, Gender, Birthdate, IdCardNr, VoterNr, Phone, Email, LocalBorn) ' +
        'VALUES (?, ?, ?, ?, ?, ?, ?, ?); SELECT * FROM person WHERE Id = LAST_INSERT_ID();', [name, gender, birthdate, docId, voterNr, phone, email, isLocalBorn])
        .then(result => { return { recordset: result, rowsAffected: [result.length] }; });
}

// function addPerson(name, gender, birthdate, docId, voterNr, phone, email, isLocalBorn) {
//     //console.log('Person to insert: ' + name + gender + birthdate + docId + voterNr + phone + email);
//     console.log('New Person: ', docId);
//     const query = function (db) {
//         const last = db.get('Person')
//             .cloneDeep()
//             .last()
//             .value();
//         const id = last ? last.Id + 1 : 1;
//         const person = { 
//             Id: id, 
//             Name: name, 
//             Gender: gender, 
//             Birthdate: birthdate, 
//             IdCardNr: docId,
//             IdCardExpireDate: null,
//             VoterNr: voterNr, 
//             Phone: phone, 
//             Email: email,
//             LocalBorn: isLocalBorn ? isLocalBorn : false,
//             CreatedAt: new Date()
//         };
//         db.get('Person')
//             .push(person)
//             .write();
//         //console.log('Add storage person:'); console.log(person);
//         return { recordset: [ person ], rowsAffected: [1] };
//     };
//     return new Promise((resolve, reject) => {
//         try {
//             const result = storage.statementQuery(query);
//             resolve(result);
//         }
//         catch(err) {
//             reject(err);
//         }
//     });
// }

function updatePerson(id, name, gender, birthdate, docId, voterNr, phone, email, isLocalBorn) {
    return mysql.executeStatement('UPDATE person SET Name = ?, Gender = ?, Birthdate = ?, ' +
            ' IdCardNr = ?, VoterNr = ?, Phone = ?, Email = ?, LocalBorn = ?) ' +
        ' WHERE Id = ?', [name, gender, birthdate, docId, voterNr, phone, email, isLocalBorn, id])
        .then(result => { return { rowsAffected: [result.affectedRows] }; });
}

// function updatePerson(id, name, gender, birthdate, docId, voterNr, phone, email, isLocalBorn) {
//     console.log('Updating Person: ', docId);
//     const query = function (db) {
//         const person = db.get('Person')
//             .find({ Id: id })
//             .assign({ 
//                 Name: name, 
//                 Gender: gender,
//                 Birthdate: birthdate, 
//                 VoterNr: voterNr, 
//                 Phone: phone, 
//                 Email: email,
//                 LocalBorn: isLocalBorn ? isLocalBorn : false,
//                 LastUpdatedAt: new Date()
//             })
//             .write();
//         //console.log('Storage person:'); console.log(person);
//         return { rowsAffected: [1] };
//     };
//     return new Promise((resolve, reject) => {
//         try {
//             const result = storage.statementQuery(query);
//             resolve(result);
//         }
//         catch(err) {
//             reject(err);
//         }
//     });
// }

module.exports = {
    getPersonById,
    getPersonByIdCardNr,
    addPerson,
    updatePerson
}
